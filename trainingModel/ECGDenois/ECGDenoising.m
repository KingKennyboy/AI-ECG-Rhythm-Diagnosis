...
clear()
PATH= 'trainingModel\ECGData\';
OutPutFilePath = 'trainingModel\ECGDataProcessed\';
%FileTable is the files you want to denoise
FileTable = (readtable('trainingModel\Diagnostics.csv','ReadVariableNames',true));
...
parfor i=1:LengthFileList
   DenFileName =strcat(FileTable.FileName{i},'.csv');
   DenDataFile = 0;
   if(~ismember(DenFileName,DenFileList))
        DataFileName = strcat(PATH,FileTable.FileName{i},'.csv');
        ...
        OutputfileName =strcat(OutPutFilePath, FileTable.FileName{i},'.csv');
        csvwrite(OutputfileName,DenoisingData);
        fprintf('Finished File: %s\n',FileTable.FileName{i});
  end
end